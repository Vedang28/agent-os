...
from agents.prompts import DEVOPS_CI_MONITOR

logger = get_logger("devops_ci.ci_monitor")


class CiMonitor:
    name = "devops_ci.ci_monitor"
    role = "proposer"

    SYSTEM_PROMPT = DEVOPS_CI_MONITOR
    ...
    async def run(self, state: AgentState) -> AgentState:
        ...
        if self._obsidian:
            ...
            for pb in get_playbooks("devops_ci", self._obsidian):
        ...
        logger.info("ci_monitor produced draft for request=%r", request[:80])
